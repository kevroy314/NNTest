    ...
    public class NN
    {
        ...
        private double[] ComputerLayerOutput(int layerNumber, double[] input)
        {
            ...
            for (int i = 0; i < output.Length; i++) //For each node
            {
                ...
                output[i] += Params.bias * weights[weightStartIndexPerLayerPerNode[layerNumber][i] + input.Length]; //Add the bias weight*(-1) (i.e. subtract the bias weight) stored in the last index given the layer number and node
                output[i] = sigmoid(output[i], Params.activationResponse);
            }
